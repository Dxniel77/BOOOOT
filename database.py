...
async def add_admin(user_id: int, username: str, first_name: str, added_by: int):
    def _f(u, un, fn, ab):
        with get_conn() as conn:
            conn.execute("INSERT OR REPLACE INTO admins (user_id,username,first_name,added_by) VALUES (?,?,?,?)", (u, un, fn, ab))
...
async def create_code(code: str, days: int, max_uses: int, note: str = "", expires_at: str = None, created_by: int = None):
    def _f(c, d, m, n, e, cb):
        with get_conn() as conn:
            conn.execute("INSERT INTO codes (code,days,max_uses,note,expires_at,created_by) VALUES (?,?,?,?,?,?)", (c, d, m, n, e, cb))
...
async def get_active_codes() -> list:
    def _f():
        with get_conn() as conn:
            return conn.execute("SELECT * FROM codes WHERE is_active=1 AND (expires_at IS NULL OR expires_at > datetime('now')) ORDER BY created_at DESC").fetchall()
    return await run(_f)
...
async def upsert_subscription(user_id: int, username: str, first_name: str, expiry: str, total_days: int, last_code: str):
...
async def add_days_to_subscription(user_id: int, days: int) -> bool:
    def _f(u, d):
        with get_conn() as conn:
            ...
            conn.execute("UPDATE subscriptions SET expiry=?, total_days=total_days+? WHERE user_id=?", (new_exp.strftime("%Y-%m-%d %H:%M:%S"), d, u))
            return True
...
async def get_active_members() -> list:
    def _f():
        with get_conn() as conn:
            return conn.execute("SELECT * FROM subscriptions WHERE expiry > datetime('now') ORDER BY expiry ASC").fetchall()
    return await run(_f)

async def get_expired_members() -> list:
    def _f():
        with get_conn() as conn:
            return conn.execute("SELECT * FROM subscriptions WHERE expiry <= datetime('now')").fetchall()
    return await run(_f)

async def get_expiring_soon(hours: int) -> list:
    def _f(h):
        with get_conn() as conn:
            return conn.execute("SELECT * FROM subscriptions WHERE expiry > datetime('now') AND expiry <= datetime('now', ? || ' hours')", (str(h),)).fetchall()
    return await run(_f, hours)

async def export_members_csv() -> str:
    def _f():
        with get_conn() as conn:
            rows = conn.execute("SELECT user_id, username, first_name, expiry, total_days, renewals, last_code, joined_at FROM subscriptions ORDER BY joined_at DESC").fetchall()
        output = io.StringIO()
        writer = csv.writer(output)
        writer.writerow(["user_id", "username", "first_name", "expiry", "total_days", "renewals", "last_code", "joined_at"])
        for row in rows:
            writer.writerow([row["user_id"], row["username"] or "", row["first_name"] or "", row["expiry"], row["total_days"], row["renewals"], row["last_code"] or "", row["joined_at"]])
...
async def ban_user(user_id: int, reason: str, banned_by: int):
    def _f(u, r, b):
        with get_conn() as conn:
            conn.execute("INSERT OR REPLACE INTO blacklist (user_id,reason,banned_by) VALUES (?,?,?)", (u, r, b))
...
async def get_stats_summary() -> dict:
    def _f():
        with get_conn() as conn:
            ...
            expiring_3d  = conn.execute("SELECT COUNT(*) FROM subscriptions WHERE expiry > datetime('now') AND expiry <= datetime('now', '3 days')").fetchone()[0]
...
async def get_user_history(user_id: int) -> list:
    def _f(u):
        with get_conn() as conn:
            return conn.execute("SELECT * FROM stats WHERE user_id=? ORDER BY created_at DESC LIMIT 30", (u,)).fetchall()
    return await run(_f, user_id)

async def audit(admin_id: int, action: str, target: str = "", detail: str = ""):
    def _f(a, ac, t, d):
        with get_conn() as conn:
            conn.execute("INSERT INTO audit_log (admin_id,action,target,detail) VALUES (?,?,?,?)", (a, ac, t, d))
    await run(_f, admin_id, action, target, detail)

async def get_audit_log(limit: int = 50) -> list:
    def _f(l):
        with get_conn() as conn:
            return conn.execute("SELECT * FROM audit_log ORDER BY created_at DESC LIMIT ?", (l,)).fetchall()
...
async def log_broadcast(message: str, filter_type: str, sent_to: int, failed: int):
    def _f(m, ft, s, f):
        with get_conn() as conn:
            conn.execute("INSERT INTO broadcast_log (message,filter_type,sent_to,failed) VALUES (?,?,?,?)", (m, ft, s, f))
    await run(_f, message, filter_type, sent_to, failed)

async def get_broadcast_history(limit: int = 10) -> list:
    def _f(l):
        with get_conn() as conn:
            return conn.execute("SELECT * FROM broadcast_log ORDER BY created_at DESC LIMIT ?", (l,)).fetchall()
    return await run(_f, limit)
...
async def create_ticket(user_id: int, username: str, first_name: str, subject: str) -> int:
    def _f(u, un, fn, s):
        with get_conn() as conn:
            cur = conn.execute("INSERT INTO support_tickets (user_id,username,first_name,subject) VALUES (?,?,?,?)", (u, un, fn, s))
...
async def get_user_tickets(user_id: int) -> list:
    def _f(u):
        with get_conn() as conn:
            return conn.execute("SELECT * FROM support_tickets WHERE user_id=? ORDER BY created_at DESC", (u,)).fetchall()
    return await run(_f, user_id)

async def get_open_tickets() -> list:
    def _f():
        with get_conn() as conn:
            return conn.execute("SELECT * FROM support_tickets WHERE status='open' ORDER BY created_at ASC").fetchall()
    return await run(_f)

async def get_all_tickets(limit: int = 30) -> list:
    def _f(l):
        with get_conn() as conn:
            return conn.execute("SELECT * FROM support_tickets ORDER BY updated_at DESC LIMIT ?", (l,)).fetchall()
    return await run(_f, limit)

async def add_ticket_message(ticket_id: int, sender_id: int, message: str, is_admin: bool = False):
    def _f(t, s, m, a):
        with get_conn() as conn:
            conn.execute("INSERT INTO ticket_messages (ticket_id,sender_id,message,is_admin) VALUES (?,?,?,?)", (t, s, m, int(a)))
            conn.execute("UPDATE support_tickets SET updated_at=datetime('now') WHERE id=?", (t,))
    await run(_f, ticket_id, sender_id, message, is_admin)

async def get_ticket_messages(ticket_id: int) -> list:
    def _f(t):
        with get_conn() as conn:
            return conn.execute("SELECT * FROM ticket_messages WHERE ticket_id=? ORDER BY sent_at ASC", (t,)).fetchall()
    return await run(_f, ticket_id)

async def close_ticket(ticket_id: int):
    def _f(t):
        with get_conn() as conn:
            conn.execute("UPDATE support_tickets SET status='closed', closed_at=datetime('now') WHERE id=?", (t,))
    await run(_f, ticket_id)

async def reopen_ticket(ticket_id: int):
    def _f(t):
        with get_conn() as conn:
            conn.execute("UPDATE support_tickets SET status='open', closed_at=NULL WHERE id=?", (t,))
    await run(_f, ticket_id)
...
async def get_ranking(limit: int = 10) -> list:
    def _f(l):
        with get_conn() as conn:
            return conn.execute("SELECT user_id, username, first_name, total_days, joined_at FROM subscriptions ORDER BY total_days DESC LIMIT ?", (l,)).fetchall()
    return await run(_f, limit)
